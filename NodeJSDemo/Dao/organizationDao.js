...
/*实体*/
var modelEtity = ['parentId','organizationName', 'organizationOrder']  //获取的字段
var queryEtity=['Id','parentId','organizationName','organizationOrder']

var dataTableName='organization'
...
function addModel(obj,fn) {
    ...
    queryModelCount(['organizationName'],obj,doSql,true) //最后一参数通知查询数量的是否使用唯一查询
...
function updateModel(obj,fn) {
    ...
    // unit.push(Date.now())
    var regex=/(?=,)|$/g;
    var upd=modelEtity.join().replace(regex,'=?')
...
function queryModel(obj,fn) {
    var doSql=function (err,count) {
        ...
        var where=obj.parentId?" WHERE parentId = "+"'"+obj.parentId+"'":0
    ...
    // doSql(0,'')
    var queryAllFilter=obj.parentId?['parentId']:[]
    queryModelCount(queryAllFilter,obj,doSql,true)
}
/*查询总量*/
